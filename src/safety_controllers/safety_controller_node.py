...
class SafetyControllerNode:
	def __init__(self):
		...
		#self.debug_pub = rospy.Publisher("/debug", String, queue_size)

		# Variables
		self.safe = 2 # 0: safe, 1: not safe, 2: scanning
		self.speed = 1
		self.curr_steering_angle = 0
		self.subsrciber_safe = True
		self.count = 0
	...
	def safety_callback(self, state):
			self.subsrciber_safe = state.data

	def laser_scan_input_callback(self, scan):
		if self.subsrciber_safe:
	...
	def in_range_for_steer_angle(self, x, y):
		if abs(self.curr_steering_angle) <= 0.2: # If car going straight
			x_range = 0.25
			y_range = self.FRONT_DIST + 1.5*self.speed
			#print y_range
		...
		elif self.curr_steering_angle > 0.2:# If car turning left
